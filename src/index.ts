...
export type IFilter<T = MetaValue> = (
    meta: T | undefined,
    similarity: number,
    tags: Tag[],
) => boolean;
...
export class TagVectorSystem<T = MetaValue> {
    ...
    exportIndex(includeItems = true): ExportedData {
        ...
        const data: ExportedData = {
            ...
            categoryWeights: Array.from(this.categoryWeights.entries()).map(
                ([category, weight]) => ({
                    category,
                    weight,
                }),
    ...
    generateQueryHash(queryTags: Tag[], _filter?: IFilter<T>): string {
        ...
        // Create hash string
        return sortedTags
            .map((tag) => `${tag.category}:${tag.value}:${tag.confidence}`)
            .join('|');
    }
    ...
    query(queryTags: Tag[], options: QueryOptions<T> = {}): QueryResult[] {
        ...
        if (this.isCacheValid(queryHash, filter)) {
            // Return all results if size is 0
            if (!size || !page) {
                return this.queryCache?.sortedResults ?? [];
            }
            return (
                this.queryCache?.sortedResults.slice((page - 1) * size, page * size) ??
                []
            );
        }
    ...
    getStats(filter?: IFilter<T>): {
        ...
        const matchingItems = filter
            ? Array.from(this.itemVectors.keys()).filter((id) =>
                filter(this.itemMeta.get(id), 0, []),
            )
    ...
    getAllCategoryWeights(): CategoryWeight[] {
        return Array.from(this.categoryWeights.entries()).map(
            ([category, weight]) => ({
                category,
                weight,
            }),
