...
describe('TagVectorSystem', () => {
  ...
  describe('Item Management', () => {
    ...
    test('should handle item removal', () => {
      ...
      const result = system.queryFirst([
        { category: 'color', value: 'red', confidence: 1.0 },
      ]);
  ...
  describe('Query Operations', () => {
    ...
    test('should handle filtering with metadata', () => {
      ...
      const filterA: IFilter<MetaType> = (_meta: MetaType | undefined): boolean => {
      ...
      const filterB: IFilter<MetaType> = (_meta: MetaType | undefined): boolean => {
        if (!_meta) return false;
        return _meta.type === 'B';
      };
  ...
  describe('Advanced Query Operations', () => {
    ...
    test('should handle query with different filters', () => {
      ...
      // First query with type A filter
      const filterA: IFilter<{ type: string }> = (_meta: { type: string } | undefined): boolean => {
        if (!_meta) return false;
        return _meta.type === 'A';
      };
      const resultsA = system.query(queryTags, { filter: filterA });
      ...
      const filterB: IFilter<{ type: string }> = (_meta: { type: string } | undefined): boolean => {
        if (!_meta) return false;
        return _meta.type === 'B';
      };
      const resultsB = system.query(queryTags, { filter: filterB });
  ...
  describe('Vector Operations', () => {
    ...
    test('should handle vectors with different dimensions', () => {
      ...
      const item2: ItemTags<MetaType> = {
        id: 'item2',
        tags: [{ category: 'size', value: 'large', confidence: 1.0 }],
        meta: { type: 'test' },
      };
      system.addItem(item1);
      system.addItem(item2);
  ...
  describe('Edge Cases', () => {
    test('should handle invalid cache states', () => {
      ...
      const filter1: IFilter<MetaType> = (_meta: MetaType | undefined): boolean => {
        return true;
      };
      system.query(queryTags, { filter: filter1 });

      const filter2: IFilter<MetaType> = (_meta: MetaType | undefined): boolean => {
        return false;
      };
    ...
    test('should handle metadata operations', () => {
      ...
      const filter: IFilter<MetaType> = (_meta: MetaType | undefined): boolean => {
        if (!_meta) return false;
        return _meta.type === 'A';
      };
    ...
    test('should handle filter with empty array', () => {
      ...
      const filter: IFilter<MetaType> = (_meta: MetaType | undefined): boolean => {
        if (!_meta) return false;
        return Array.isArray(_meta.tags) && _meta.tags.length === 0;
      };
      const queryTags: Tag[] = [
        { category: 'color', value: 'red', confidence: 1.0 },
      ];
